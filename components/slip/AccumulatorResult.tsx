  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {/* Top row: big probability + risk badge */}
        <div className="flex items-start justify-between mb-3">
          <div>
            <p className="text-xs text-white/40 font-medium uppercase tracking-widest mb-1">
              Combined Probability
            </p>
            <p className="text-4xl font-black tabular-nums leading-none" style={{ color: risk.colour }}>
              {prob.toFixed(1)}%
            </p>
          </div>
          <span
            className="px-3 py-1.5 rounded-full border text-xs font-bold mt-1"
        ...
        <div className="space-y-2 pt-3 border-t border-white/8">
          <div className="flex items-center justify-between">
            <span className="text-xs text-white/40">Theoretical parlay odds</span>
          ...
          {Math.abs(analysis.aiOverallScore - prob) >= 2 && (
            <div className="flex items-center justify-between">
              <span className="text-xs text-white/40">AI holistic score</span>
              <span className="text-sm font-mono font-semibold text-white/70">
      ...
      <div>
        ...
        <div className="space-y-2">
          {analysis.picks.map((pick) => {
            ...
            const colour = p >= 65 ? "#22c55e" : p >= 45 ? "#eab308" : p >= 30 ? "#f97316" : "#ef4444";
